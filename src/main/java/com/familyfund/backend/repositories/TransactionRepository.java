...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
        ...
        // Obtener la suma de ingresos o gastos totales por mes
        //Con COALESCE nos devolverá 0 si no hay transacciones ese mes
        @Query("SELECT COALESCE(SUM(t.amount), 0) " +
                        "FROM Transaction t " +
                        "WHERE t.usuario.family.id = :familyId " +
                        "AND t.type = :type " +
                        "AND t.year = :year " +
                        "AND t.month = :month")
        double sumByFamilyAndTypeAndMonth(
                        @Param("familyId") Long familyId,
